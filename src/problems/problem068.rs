...
fn fill_ring(ring: &mut [u64; 15], i: usize, first: u64, used: &mut HashSet<u64>, results: &mut Vec<u64>) {
    match i {
        15 => {
            if line_sum(ring, 0) == line_sum(ring, 3) &&
               line_sum(ring, 3) == line_sum(ring, 9) &&
        ...
        _ => {
            ...
            for x in start..11 {
                ...
                ring[i] = x;
                used.insert(x);
                fill_ring(ring, i + 1, (if i == 0 { x } else { first }), used, results);
                used.remove(&x);
            }
        },
    }
