...
pub fn solve() -> usize {
    ...
    // find cyclic
    edges.keys().filter_map(|&high| find_cyclic(high, high, &edges, &mut HashSet::new(), &mut HashSet::new()) ).nth(0).unwrap().into_iter().summation()
}

fn find_cyclic(first: usize, high: usize, edges: &Edges, polys: &mut HashSet<usize>, nums: &mut HashSet<usize>) -> Option<HashSet<usize>> {
