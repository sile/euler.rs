...
pub fn solve() -> usize {
    let grid: [[usize; 20]; 20] = [[08, 02, 22, 97, 38, 15, 00, 40, 00, 75, 04, 05, 07, 78, 52,
                                    12, 50, 77, 91, 08],
                                   ...
                                   [32, 98, 81, 28, 64, 23, 67, 10, 26, 38, 40, 67, 59, 54, 70,
                                    66, 18, 38, 64, 70],
                                   [67, 26, 20, 68, 02, 62, 12, 20, 95, 63, 94, 39, 63, 08, 40,
                                    91, 66, 49, 94, 21],
                                   ...
                                   [88, 36, 68, 87, 57, 62, 20, 72, 03, 46, 33, 67, 46, 55, 12,
                                    32, 63, 93, 53, 69],
                                   [04, 42, 16, 73, 38, 25, 39, 11, 24, 94, 72, 18, 08, 46, 29,
                                    32, 40, 62, 76, 36],
    ...
    for y in 0..20 {
        for x in 0..17 {
            products.push((0..4).map(|i| grid[y][x + i]).prod());
        }
    }
    // vertical
    for x in 0..20 {
        for y in 0..17 {
            products.push((0..4).map(|i| grid[y + i][x]).prod());
        }
    }
    // diagonal-1
    for y in 0..17 {
        for x in 0..17 {
            products.push((0..4).map(|i| grid[y + i][x + i]).prod());
        }
    }
    // diagonal-2
    for y in 0..17 {
